...
TRD_Tracker::TRD_Tracker(int n, int s, float mi)
{
    nfeatures = n;
    Dictionary_size = s;
    minHessian = mi;
...
void TRD_Tracker::TrackerInitialization(Mat img, Rect init_rect)
{
    ...
    DictionaryInitialization(img, Static_Dictionary, minHessian, init_rect);
    DictionaryInitialization(img, Confidence_Dictionary, minHessian, init_rect);
    DictionaryInitialization(img, Dynamitic_Dictionary, minHessian, init_rect);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// \brief TRD_Tracker::ORB/FAST_Detection
/// \param image
/// \param mi (minHessian)
/// \return keypoint
/// detect feature points (Orb or Fast)

std::vector<cv::KeyPoint> TRD_Tracker::ORB_Detection(cv::Mat image, int mi)
{
    cv::OrbFeatureDetector detector( mi );

    std::vector<cv::KeyPoint> keypoints_1;

    detector.detect( image, keypoints_1 );

    while(keypoints_1.size() < nfeatures && mi > 10)
    {
        keypoints_1.clear();
        mi = mi * 9 / 10;
        cv::OrbFeatureDetector detector( mi );
    ...
    if(keypoints_1.size() > 2*nfeatures)
        cv::KeyPointsFilter::retainBest(keypoints_1, 2*nfeatures);
...
{
    cv::FastFeatureDetector detector( mi );
    std::vector<cv::KeyPoint> keypoints_1;
    keypoints_1.clear();
    detector.detect( image, keypoints_1 );
    while(keypoints_1.size() < nfeatures && mi > 10)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// \brief TRD_Tracker::FAST/ORB_Extraction
/// \param image
/// \param keypoint
/// \return descriptor matrix
/// extract ORB feature

void TRD_Tracker::ORB_Extraction(cv::Mat image, std::vector<cv::KeyPoint>& keypoint, cv::Mat& descriptors)
{
...
void TRD_Tracker::DictionaryInitialization(cv::Mat image, Dictionary& D, int mi, cv::Rect init_rect)
{
    ...
    D.keypoints = FAST_Detection(image(Searching_rect), mi);
...
//    ORB_Extraction(image, D.keypoints, D.descriptors);
...
void TRD_Tracker::FeatureDetecting(Dictionary D)
{
    candidate.clear();
...
//    candidate = ORB_Detection(image_patch, minHessian);
    candidate = FAST_Detection(image_patch, minHessian);
...
//    ORB_Extraction(current_frame, candidate, candidate_descriptors);
}
...
void TRD_Tracker::FeatureScaleUpdate()
{
    if(good_matches.size() > 15)
    {
        ...
        scale[0] = (cx_div / dx_div - 1) * 0.1 + 1;
        scale[1] = (cy_div / dy_div - 1) * 0.1 + 1;
        for(uint i = 0; i < Dynamitic_Dictionary.keypoints.size(); i ++)
        {
            Dynamitic_Dictionary.keypoints[i].pt.x = (int)(scale[0] * ( (float)Dynamitic_Dictionary.keypoints[i].pt.x - dx_ave ) + dx_ave);
            Dynamitic_Dictionary.keypoints[i].pt.y = (int)(scale[1] * ( (float)Dynamitic_Dictionary.keypoints[i].pt.y - dy_ave ) + dy_ave);
        }
    }
    else
    {
...
void TRD_Tracker::DynamiticDictionaryUpdating()
{
    ...
    {
        DictionaryInitialization(current_frame, Confidence_Dictionary, minHessian, Dynamitic_Dictionary.tracking_rect);
    }
    if(recover_flag == 2)
    {
        DictionaryInitialization(current_frame, Confidence_Dictionary, minHessian, Dynamitic_Dictionary.tracking_rect);
...
void TRD_Tracker::FeatureMatchingTest(cv::Mat img)
{
    ...
    FeatureDetecting(Dynamitic_Dictionary);
    ...
    if(candidate.size() > 0)
    {
        ...
        std::cout << "Position shift has been found..." << std::endl;
        FeatureScaleUpdate();
