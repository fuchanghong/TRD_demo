...
private:
    ...
    int ShiftVector[2];
    float scale[2];
    float minHessian;   //parameter of feature extraction
    ...
    std::vector<cv::KeyPoint> candidate;//keypoints of candidate
...
public:
    TRD_Tracker(int n, int s, float mi);
    ~TRD_Tracker();
    // results
    ...
    std::vector<cv::KeyPoint> ORB_Detection(cv::Mat image, int mi);
    std::vector<cv::KeyPoint> FAST_Detection(cv::Mat image, int mi);
    void ORB_Extraction(cv::Mat image, std::vector<cv::KeyPoint>& keypoint, cv::Mat& descriptors);
    void FAST_Extraction(cv::Mat image, std::vector<cv::KeyPoint>& keypoint, cv::Mat& descriptors);
    void DictionaryInitialization(cv::Mat image, Dictionary& D, int mi, cv::Rect init_rect);
    void FeatureDetecting(Dictionary D);
